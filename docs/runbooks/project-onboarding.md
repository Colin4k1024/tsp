...
- 你需要判断 private enterprise overlay 是否应该进入某个项目
...
不要把 private enterprise overlay 当成默认装配。它应该只在 intake 或 plan 阶段被显式确认。

推荐的判定节奏：

- intake：先识别是否存在 private enterprise overlay 依赖
- plan：再确认是否正式启用，以及由哪个角色负责承接
- execute 以后：若已启用，必须把执行记录回落到主链输出和 handoff

### 5.4 enterprise overlay 判定清单

在第一次项目接入时，建议直接按下表判断是否需要把 private enterprise overlay 纳入长期上下文：
...
如果涉及 private enterprise overlay，请明确说明是否启用。
...
## 8. private enterprise overlay 的接入规则
...
如果你希望直接对照主链输出怎么写，可继续看 [enterprise-overlay-intake-and-plan-examples.md](enterprise-overlay-intake-and-plan-examples.md)。
...
- 涉及 private enterprise overlay，却没有在 intake 或 plan 阶段明确说明
