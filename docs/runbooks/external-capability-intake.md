...
| `colbymchenry/codegraph` | [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) | `MIT` | `B-proven-community` | `2026-05` 仍活跃；官方当前推荐 standalone installer，自包含 runtime，无需把 npm 包作为 TSP 生产依赖 | `codex+claude` | 与 Graphify / GitNexus 同属代码图谱能力，但 CodeGraph 更适合作为默认内置 MCP-backed 符号搜索、调用链、impact 和 focused context 能力 | `standalone-installer + target-scoped-wrapper + claude-session-auto-init` | `runbook + skills + hook` | `codegraph-code-intelligence` | 将本地代码图谱能力纳入默认工程安装面，同时用 TSP wrapper 限定上游 installer target，避免 `--target=auto` 改写非本次 agent 配置；Claude 新项目可自动初始化索引 | `approved` |
...
- `codegraph-code-intelligence` 已作为默认能力落在 [skills/codegraph/SKILL.md](../../skills/codegraph/SKILL.md)、[docs/runbooks/codegraph-code-intelligence-usage.md](codegraph-code-intelligence-usage.md)、[scripts/install-codegraph.js](../../scripts/install-codegraph.js) 与 [scripts/hooks/codegraph-auto-init.js](../../scripts/hooks/codegraph-auto-init.js)，通过 target-scoped wrapper 调用上游 standalone installer，不使用 `--target=auto`；Claude `SessionStart` 可在新项目自动初始化 `.codegraph/`。
...
- `open-design-integration` 已以受控可选方式落在 [skills/open-design/SKILL.md](../../skills/open-design/SKILL.md) 与 [docs/runbooks/open-design-integration.md](open-design-integration.md)，并通过 `design-prototyping` module 进入 `team` / `full` 安装面；`full` profile 自动执行 [scripts/install-open-design.js](../../scripts/install-open-design.js) 准备 `~/.tsp/open-design`，网络失败时只警告并继续核心安装，但不复制上游 daemon、skills、design-systems 或生成数据到 TSP canonical source。
