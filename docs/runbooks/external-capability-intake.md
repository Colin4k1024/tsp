...
| `colbymchenry/codegraph` | [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) | `MIT` | `B-proven-community` | `2026-05` 仍活跃；npm 包 `0.7.12`；README 覆盖 CLI、MCP tools、installer、Node `>=18 <25` 与多语言 tree-sitter 图谱能力 | `codex+claude` | 与 Graphify / GitNexus 同属代码图谱能力，但 CodeGraph 更适合作为默认内置 MCP-backed 符号搜索、调用链、impact 和 focused context 能力 | `default-dependency + upstream-installer-wrapper` | `runbook + skills` | `codegraph-code-intelligence` | 将本地代码图谱能力纳入默认工程安装面，同时用 TSP wrapper 限定上游 installer target，避免 `--target=auto` 改写非本次 agent 配置 | `approved` |
