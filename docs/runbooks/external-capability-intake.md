...
| `colbymchenry/codegraph` | [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) | `MIT` | `B-proven-community` | `2026-05` 仍活跃；TSP 锁定 npm 包 `0.7.10` 以保持 Node `>=18 <25` 兼容；上游 `0.8.0` 当前要求 Node `>=20 <25` | `codex+claude` | 与 Graphify / GitNexus 同属代码图谱能力，但 CodeGraph 更适合作为默认内置 MCP-backed 符号搜索、调用链、impact 和 focused context 能力 | `default-dependency + upstream-installer-wrapper` | `runbook + skills` | `codegraph-code-intelligence` | 将本地代码图谱能力纳入默认工程安装面，同时用 TSP wrapper 限定上游 installer target，避免 `--target=auto` 改写非本次 agent 配置 | `approved` |
...
- `codegraph-code-intelligence` 已作为默认依赖能力落在 [skills/codegraph/SKILL.md](../../skills/codegraph/SKILL.md)、[docs/runbooks/codegraph-code-intelligence-usage.md](codegraph-code-intelligence-usage.md) 与 [scripts/install-codegraph.js](../../scripts/install-codegraph.js)，通过 target-scoped wrapper 调用上游 installer，不使用 `--target=auto`，不在安装时初始化 `.codegraph/`。
