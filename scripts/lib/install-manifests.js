...
const SUPPORTED_INSTALL_TARGETS = ['claude', 'cursor', 'antigravity', 'codex', 'gemini', 'opencode', 'cangming', 'codebuddy', 'copilot', 'windsurf', 'augment'];
...
const DEFAULT_OVERLAY_PACKAGE_BY_ID = Object.freeze({
  enterprise: '@<private-scope>/tsp-enterprise-overlay',
});
...
function mergeObjectsById(baseEntries, overlayEntries) {
  const merged = new Map();
  for (const entry of Array.isArray(baseEntries) ? baseEntries : []) {
    merged.set(entry.id, entry);
  }
  for (const entry of Array.isArray(overlayEntries) ? overlayEntries : []) {
    merged.set(entry.id, entry);
  }
  return [...merged.values()];
}

function resolveOverlayPackageRoot(overlayId, options = {}) {
  const normalizedOverlayId = String(overlayId || '').trim();
  const explicitRoot = options.overlayPackageMap && options.overlayPackageMap[normalizedOverlayId];
  if (explicitRoot) {
    return path.resolve(explicitRoot);
  }

  const envKey = `ECC_INSTALL_OVERLAY_${normalizedOverlayId.toUpperCase().replace(/-/g, '_')}_PATH`;
  if (process.env[envKey]) {
    return path.resolve(process.env[envKey]);
  }

  const packageName = (options.overlayPackageNames && options.overlayPackageNames[normalizedOverlayId])
    || process.env[`ECC_INSTALL_OVERLAY_${normalizedOverlayId.toUpperCase().replace(/-/g, '_')}_PACKAGE`]
    || DEFAULT_OVERLAY_PACKAGE_BY_ID[normalizedOverlayId];
  if (!packageName) {
    throw new Error(`Unknown install overlay: ${normalizedOverlayId}`);
  }

  try {
    const packageJsonPath = require.resolve(`${packageName}/package.json`, {
      paths: [options.repoRoot || DEFAULT_REPO_ROOT, options.cwd || process.cwd()],
    });
    return path.dirname(packageJsonPath);
  } catch (_error) {
    throw new Error(
      `Unable to resolve ${normalizedOverlayId} overlay package (${packageName}). Install the private package or set ${envKey}.`
    );
  }
}

function loadOverlayManifestData(overlayId, options = {}) {
  const overlayRoot = resolveOverlayPackageRoot(overlayId, options);
  const overlayConfigPath = path.join(overlayRoot, 'ecc-overlay.json');
  const overlayConfig = fs.existsSync(overlayConfigPath)
    ? readJson(overlayConfigPath, `${overlayId} overlay config`)
    : {
      version: 1,
      id: overlayId,
      manifests: {
  ...
  const manifestsConfig = overlayConfig.manifests || {};
  const modulesPath = path.join(overlayRoot, manifestsConfig.modulesPath || 'manifests/install-modules.json');
  const profilesPath = path.join(overlayRoot, manifestsConfig.profilesPath || 'manifests/install-profiles.json');
  const componentsPath = path.join(overlayRoot, manifestsConfig.componentsPath || 'manifests/install-components.json');

  if (!fs.existsSync(modulesPath) || !fs.existsSync(profilesPath)) {
    throw new Error(`Overlay ${overlayId} is missing manifest files under ${overlayRoot}`);
  }

  const modulesData = readJson(modulesPath, `${overlayId} install-modules.json`);
  const profilesData = readJson(profilesPath, `${overlayId} install-profiles.json`);
  const componentsData = fs.existsSync(componentsPath)
    ? readJson(componentsPath, `${overlayId} install-components.json`)
...
function loadInstallManifests(options = {}) {
  ...
  const componentsData = fs.existsSync(componentsPath)
    ...
    : { version: null, components: [] };
  const overlayIds = dedupeStrings(options.overlayIds);
  let modules = Array.isArray(modulesData.modules) ? modulesData.modules : [];
  let profiles = profilesData && typeof profilesData.profiles === 'object'
    ? profilesData.profiles
    : {};
  let components = Array.isArray(componentsData.components) ? componentsData.components : [];
  const overlays = [];

  for (const overlayId of overlayIds) {
    const overlay = loadOverlayManifestData(overlayId, options);
    overlays.push({
      id: overlay.overlayId,
      root: overlay.overlayRoot,
    });
    modules = mergeObjectsById(modules, Array.isArray(overlay.modulesData.modules) ? overlay.modulesData.modules : []);
    profiles = {
      ...profiles,
      ...(overlay.profilesData && typeof overlay.profilesData.profiles === 'object'
        ? overlay.profilesData.profiles
        : {}),
    };
    components = mergeObjectsById(
      components,
      Array.isArray(overlay.componentsData.components) ? overlay.componentsData.components : []
    );
  }
...
function resolveInstallPlan(options = {}) {
  ...
  return {
    ...
    skippedModules,
    excludedModules,
    overlayIds: manifests.overlayIds,
