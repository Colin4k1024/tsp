...
const LEGACY_INSTALL_TARGETS = ['claude', 'grok', 'antigravity', 'codex', 'opencode', 'cangming'];
...
function parseInstallArgs(argv) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--target') {
      ...
      const overlayId = args[index + 1] || '';
      if (overlayId.trim()) {
        parsed.overlayIds.push(overlayId.trim());
      }
      index += 1;
    } else if (arg === '--with') {
...
function normalizeInstallRequest(options = {}) {
  const config = options.config && typeof options.config === 'object'
    ? options.config
    : null;
  const overlayIds = dedupeStrings([
    ...(config?.overlayIds || []),
    ...(options.overlayIds || []),
  ]);
  ...
  const moduleIds = validateInstallModuleIds(
    dedupeStrings([...(config?.moduleIds || []), ...(options.moduleIds || [])]),
    {
      ...
      cwd: options.cwd,
      overlayPackageMap: options.overlayPackageMap,
    }
  );
