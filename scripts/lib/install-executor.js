...
function getPackageVersion(sourceRoot) {
  try {
    const raw = fs.readFileSync(path.join(sourceRoot, 'package.json'), 'utf8');
    let packageJson;
    try {
      packageJson = JSON.parse(raw);
    } catch (parseErr) {
      throw new Error(`Failed to parse package.json: ${parseErr.message || parseErr}`);
    }
...
function getManifestVersion(sourceRoot) {
  try {
    const raw = fs.readFileSync(path.join(sourceRoot, 'manifests', 'install-modules.json'), 'utf8');
    let modulesManifest;
    try {
      modulesManifest = JSON.parse(raw);
    } catch (parseErr) {
      throw new Error(`Failed to parse install-modules.json: ${parseErr.message || parseErr}`);
    }
...
function createManifestInstallPlan(options = {}) {
  ...
  const overlayIds = Array.isArray(options.overlayIds) ? [...options.overlayIds] : [];
