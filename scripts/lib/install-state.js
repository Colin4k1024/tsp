  ...
  const validate = state => {
    ...
    if (!request || typeof request !== 'object' || Array.isArray(request)) {
      ...
      validateNoAdditionalProperties(
        request,
        '/request',
        ['profile', 'modules', 'overlays', 'includeComponents', 'excludeComponents', 'legacyLanguages', 'legacyMode']
      );
      ...
      validateStringArray(request.overlays, '/request/overlays');
...
function createInstallState(options) {
  ...
  const state = {
    ...
    request: {
      ...
      overlays: Array.isArray(options.request.overlays) ? [...options.request.overlays] : [],
