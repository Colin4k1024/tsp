...
} = require('./install-audit-manifest');
const {
...
function analyzeRecord(record, context) {
  ...
  const installManifestPath = deriveInstallManifestPath(record.installStatePath);
  const manifestHealth = verifyInstallAuditManifest(installManifestPath);
  for (const manifestIssue of manifestHealth.issues || []) {
    issues.push(buildIssue(
      manifestIssue.severity || 'warning',
      manifestIssue.code || 'install-manifest-issue',
      manifestIssue.message || 'install-manifest issue detected',
      {
        installManifestPath,
      },
    ));
  ...
  if (
    manifestHealth.manifest
    && manifestHealth.manifest.target
    && manifestHealth.manifest.target.installStatePath
    && manifestHealth.manifest.target.installStatePath !== record.installStatePath
  ) {
    issues.push(buildIssue(
      'warning',
      'install-manifest-state-path-mismatch',
      `install-manifest references install-state path ${manifestHealth.manifest.target.installStatePath}, expected ${record.installStatePath}`,
      {
        installManifestPath,
      },
    ));
  ...
  if (!state.request.legacyMode) {
    try {
      const desiredPlan = resolveInstallPlan({
        ...
        overlayIds: state.request.overlays || [],
  ...
  return {
    ...
    installManifestPath,
    installManifest: {
      status: manifestHealth.status,
      checkedFileCount: manifestHealth.checkedFileCount || 0,
      missingFileCount: manifestHealth.missingFileCount || 0,
      mismatchedFileCount: manifestHealth.mismatchedFileCount || 0,
    },
    issues,
  };
}

function detectKnownRisks(context) {
  const risks = [];

  try {
    const codexFullPlan = resolveInstallPlan({
      repoRoot: context.repoRoot,
      projectRoot: context.projectRoot,
      homeDir: context.homeDir,
      target: 'codex',
    ...
    if (Array.isArray(codexFullPlan.skippedModuleIds) && codexFullPlan.skippedModuleIds.includes('commands-core')) {
      risks.push(buildIssue(
        'error',
        'codex-commands-core-regression',
        'Regression detected: target codex with profile full skipped commands-core. Investigate manifest target coverage or adapter planning drift.',
      ));
    }
  } catch (_error) {
    // Keep doctor resilient when profile manifests evolve.
  }
...
function buildDoctorReport(options = {}) {
  ...
  const knownRisks = detectKnownRisks(context);
  ...
  summary.knownRiskCount = knownRisks.length;
  ...
  const results = records.map(record => {
    if (record.error) {
      return {
        ...
        installManifestPath: deriveInstallManifestPath(record.installStatePath),
        repairedPaths: [],
        plannedRepairs: [],
        error: record.error,
      };
    ...
    try {
      ...
      const installManifestPath = deriveInstallManifestPath(desiredPlan.installStatePath);
      ...
      writeInstallAuditManifest(desiredPlan, { installManifestPath });
  ...
  const results = records.map(record => {
    ...
    try {
      ...
      if (fs.existsSync(installManifestPath)) {
        ...
        removedPaths.push(installManifestPath);
        cleanupTargets.push(installManifestPath);
      }
