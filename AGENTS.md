...
- 代码图谱能力：`skills/codegraph/` + [codegraph-code-intelligence-usage.md](docs/runbooks/codegraph-code-intelligence-usage.md) 用于默认内置 MCP-backed 符号搜索、调用链和影响面证据；`skills/graphify/` + [graphify-knowledge-graph-usage.md](docs/runbooks/graphify-knowledge-graph-usage.md) 用于轻量 brownfield 结构扫描；`skills/gitnexus/` + [gitnexus-code-intelligence-usage.md](docs/runbooks/gitnexus-code-intelligence-usage.md) 用于受控 MCP 查询、impact、detect_changes 与多仓图谱证据。
- 能力边界：CodeGraph / Graphify / GitNexus 都只作为分析证据层，不替代 `/team-*` 主链；TSP 通过 target-scoped wrapper 调用 CodeGraph 官方 standalone installer，不使用 `--target=auto`；Claude `SessionStart` 可在新项目静默执行 `codegraph init -i`，Codex/OpenCode 不做侵入式自动 hook。
...
# [harness-public] recent context, 2026-06-10 5:47pm GMT+8

Legend: 🎯session 🔴bugfix 🟣feature 🔄refactor ✅change 🔵discovery ⚖️decision 🚨security_alert 🔐security_note
Format: ID TIME TYPE TITLE
Fetch details: get_observations([IDs]) | Search: mem-search skill

Stats: 50 obs (10,941t read) | 676,485t work | 98% savings
...
### May 8, 2026
2919 3:04p 🔴 npmjs 最新版本无法通过 npx 安装 — 问题发现
2921 " 🔵 @colin4k1024/tsp npx 失败根因 — bin 字段缺少与包名匹配的 `tsp` 入口
2922 3:05p 🔴 @colin4k1024/tsp npx 入口修复 — 新增 tsp bin 入口并发布 v2.4.3
S1715 @colin4k1024/tsp npmjs 最新版本无法通过 npx 安装 — 诊断根因并修复发布 (May 8 at 3:05 PM)
S1713 @colin4k1024/tsp npx 入口修复 — 新增 tsp bin 入口并发布 v2.4.3 (May 8 at 3:05 PM)
S1718 @colin4k1024/tsp v2.4.3 npm 发布 — 需要 OTP 验证码完成发布 (May 8 at 3:05 PM)
S1723 @colin4k1024/tsp v2.4.3 发布到 npmjs — 因账户启用 2FA 被 OTP 拦截，等待用户提供 OTP 或 Automation Token (May 8 at 3:07 PM)
2924 3:10p ⚖️ GitHub 推送采用浏览器授权模式
2925 " 🔵 @colin4k1024/tsp@2.4.3 发布被 OTP 二步验证阻断
2927 3:11p 🔵 npm --auth-type=web 无法绕过 publish OTP 验证
S1724 npm Token Exposed in Plain-Text Shell Command (May 8 at 3:11 PM)
2932 3:14p 🚨 npm Token Exposed in Plain-Text Shell Command
S1725 @colin4k1024/tsp v2.4.3 npm publish 失败根因分析 — Classic token 仍需 2FA OTP，需改用 Automation token (May 8 at 3:14 PM)
S1731 @colin4k1024/tsp npm publish blocked — root cause identified as package-level mfa=publish setting, OTP required to unblock (May 8 at 3:14 PM)
2935 3:17p 🚨 npm Token Exposed in Primary Session User Request
2936 " 🔵 harness-public npm Publish Blocked by 2FA (EOTP)
2937 3:18p 🔵 @colin4k1024/tsp Registry State — Published Versions Confirmed
2942 3:20p 🔵 npm 2FA OTP 未设置 — 用户确认未配置 OTP
2945 3:22p 🔵 npm 账户无 Automation Token 选项 — Classic Token 类型受限
S1735 npm 账户无 Automation Token 选项 — Classic Token 类型受限 (May 8 at 3:22 PM)
