...
|------|---------|----------|-----------|
| **No-verify blocker** | `Bash` | Blocks `git commit --no-verify` so local Git hooks cannot be bypassed | 2 (blocks) |
...
| **PUA success reset** | `Bash\|Write\|Edit\|MultiEdit` | Resets PUA consecutive-failure state after successful execution |
...
| **PUA failure escalation** | `*` via `PostToolUseFailure` | Increments failure count, assigns L1-L4, and records pressure state |
...
| **PUA journal sync** | `Stop` | Persists current PUA pressure state and recent failure summary |
